...
; procedures.
; 2004-04-23: Modified gain_off_1ff to use a FIXED gain and only 
...
;====================================================================
pro off_only_1ff, Di, astrom, astrom0, map_no, Wi, n_terms, Gp, Fp, Fq, $
...
Sa = fltarr(Px,Py,M)
for i=0L,n_elements(h_sky)-1 do Sa[*,*,h_sky[i]] = $
...
delHD = fltarr(P + M + Gamma)
delHD[0:P-1] = total(Di_H0i,3,/double)
for i=0L,M-1 do delHD[P+i] = total(Di_H0i[*,*,i],/double)
...
for i=0L,Gamma-1 do begin
  ...
  delHD[P+M+i] = total(Gp[here mod P]*Di_H0i[here],/double)
  pix_ind[here] = i
...
for i=0L,M-1 do U[*,*,i] = irootA*U[*,*,i]*irootJ[i]
...
if (do_ipsimap) then begin
  ...
  for i=0L,Gamma-1 do begin
    ...
    for j=0L,n_elements(here)-1 do begin
      ...
      UtT[*] = UtT[*] + U[pix_x,pix_y,*] * T[here[j]]
    endfor
    q_here = here mod P
...
status = call_external(c_dir+'spax_tau_1ff.so','spax_tau_1ff',tau,pix_ind,$
  irootC*delHD[P+M:*],zeta0,P,PxM)
zeta0 = -zeta0 + irootJ*delHD[P:P+M-1]
...
while ( ((abs(sZ[i]) gt sZ_limit) and (abs(sZ2[i]) gt sZ2_limit) and $
  ...
  status = call_external(c_dir+'spax_u_1ff.so','spax_u_1ff',U,q_ind,$
                          zetan,Uzetan,P,PxM)
  status = call_external(c_dir+'sptx_u_1ff.so','sptx_u_1ff',U,q_ind,Zn,UtZn,P,PxM)
...
help,/mem
time1 = systime(1)
print,' Total time (hr) = ',(time1-time0) / 3600.

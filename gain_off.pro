...
;====================================================================
pro gain_off, Di, astrom, astrom0, map_no, Wi, n_terms, Gp, Fp, Fq, $
...
for i=0L,Gamma-1 do begin
  ...
  delHD[2*P+i] = total(Gp[here mod P]*Di_H0i[here],/double)
  pix_ind[here] = i
...
for i=0L,2*M-1 do T[*,*,i] = irootA[*,*,i/M]*T[*,*,i]*Gp $
...
;NOT OFFSETS!  TTtZn[P:*] = TTtZn[P:*] - total(TTtZn[P:*],/double)/P ; norm
  ...
  if (do_psimap) then begin
    ...
    Tzetan = fltarr(2*P)
    status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
                        zetan,Tzetan,2*P,2*PxM)
    status = call_external(c_dir+'sptx.so','sptx',T,pix_ind2,$
                          Tzetan,TtTzetan,2*P,2*PxM)

...
;====================================================================
pro gain_off_1sky, Di, astrom, astrom0, map_no, Wi, n_terms, Gp, Fp, Fq, $
          skymap, sigmadet, sigmasky, cr_flag, dark_flag, $
          n_sig=n_sig, scale=scale, term_limits=term_limits, $
          pix_no=pix_no, xxs=xxs, yys=yys, xo=xo, yo=yo, $; goodpix=goodpix, $
...
; goodpix    = (Px,Py) array of known good pixels (1) and bad pixels (0)
;              This can be used to help ensure bad pixels are really ignored. 
...
;*******************
;;- ketron's edits
if n_tags(skymap) ge 1 then begin
   for i = 0, n_tags(skymap) - 1 do begin
      h = where(wtmap.(i) lt 0.1*max(wtmap.(i)),compl=compl)
      skymap.(i)[h] = median(skymap.(i)[compl])
      wG = 0      
   endfor
   if n_elements(Fq) eq 1 then Fq = fltarr(M)
;*******************
endif else begin
   h = where(wtmap lt 0.1*max(wtmap),compl=compl)
   skymap[h] = median(skymap[compl])
   wG = 0
endelse
...
for i=0L,Px-1 do for j=0L,Py-1 do begin
  Gp_A[i,j] = total((Sa[i,j,*]+Fq)^2*Wi[i,j,*],/double)
  Hp_A[i,j] = total((Sa[i,j,*]+Fq)*Wi[i,j,*],/double)
endfor
...
irootJ = fltarr(M)
for i=0L,M-1 do irootJ[i] = 1./sqrt(total(Gp^2*Wi[*,*,i],/double))

delHD = fltarr(2*P + M + Gamma)
i0 = lindgen(M)*P
for i=0L,P-1 do delHD[i] = total((Sa[i0+i]+Fq)*Di_H0i[i0+i],/double)
delHD[P:2*P-1] = total(Di_H0i,3,/double)
for i=0L,M-1 do delHD[2*P+i] = total(Gp*Di_H0i[*,*,i],/double)
...
for i=0L,M-1 do T[*,*,i] = T[*,*,i]+Fq[i]*Wi[*,*,i]
tau = temporary(Wi)
U = T
...
for i=0L,2*M-1 do U[*,*,i] = $
      (irootA[*,*,i/M]*U[*,*,i] + irootA[*,*,i/M+2]*U[*,*,(i+M)mod(M*2)]) $
             *replicate(irootJ[i mod M],Px,Py)*Gp
for i=0L,M-1 do tau[*,*,i] = replicate(irootJ[i],Px,Py)*tau[*,*,i]*Gp^2
for i=0L,Gamma-1 do begin
  here = pix_sort[starts[i]:starts[i+1]-1]
...
;  zeta0[4*lindgen(M)+ii] - total(zeta0[4*lindgen(M)+ii],/double)/M  ;normalize
zeta0 = float(zeta0)
zetan = zeta0
...
while ( ((abs(sZ[i]) gt sZ_limit) and (abs(sZ2[i]) gt sZ2_limit) and $
  ...
  status = call_external(c_dir+'spax_u_1ff.so','spax_u_1ff',U,q_ind,$
                          zetan,Uzetan,2*P,2*PxM)
  status = call_external(c_dir+'sptx_u_1ff.so','sptx_u_1ff',U,q_ind,Zn,UtZn,2*P,2*PxM)
...
;NOT OFFSETS!  TTtZn[P:*] = TTtZn[P:*] - $
;    total(TTtZn[P:*],/double)/P ; normalization
;?   TTtZn[0:P-1] = TTtZn[0:P-1] - total(TTtZn[0:P-1],/double)/P ; normalize
...
make_sigs,Di,pix_no,pix_sort,map_no,xxs,yys,Gp,Fp,skymap,Wi,cr_flag,$
  sigmadet,sigmasky,chi2,varsky=Fq,n_sig=n_sig ;, goodpix=goodpix
skymap = make_maps(Di,pix_no,pix_sort,xxs,yys,Gp,Fp,Wi*cr_flag,varsky=Fq,wtmap)

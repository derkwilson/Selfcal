...
; 1999-11-17
;
; 2000-03-08 ; made minor speed improvement in make_map
; 2000-04-07 ; inserted new rms sigma calculations & CR flagging code
;            ; fixed umap (but not psimap!) calculations to handle quadrants
; 2000-04-10 ; fixed psimap calculations (I think). It's hard to tell
;            ; if umap and psimap are really correct.
; 2001-01-05 ; added calculation of diagonal of weights (Psi^(-1)) matrix.
; 2001-12-19 ; Added simple direct Chi^2 calculation to make_sig
;            ; Converted x(i,j) index format to x[i,j] index format (IDL 5) 
;            ; Slight speed improvements in the while loop, partly at 
;            ; the expense of memory (pix_ind2)
; Version: 2002-03-29
...
; 2002-10-07: Fixed x-y dimension bug in delHD and irootj calculation that
; caused code to crash on non-square arrays. Unique to this version.
; However sp*x_tau routines still assume square arrays.
; 2002-10-21: Changes for reduced memory and added use of scratch file.
;====================================================================
pro gain_off_quad, Di, astrom, astrom0, map_no, Wi, n_terms, Gp, Fp, Fq, $
...
; Fq         = stack (2,2,M) of variable offsets per quadrant per frame
;OUTPUT:
...
for i=0L,M-1 do begin
  Di_H0i[*,*,i] = Wi[*,*,i] * cr_flag[*,*,i] * $
       (Di[*,*,i]-Gp*Sa[*,*,i]-Fp-congrid(Fq[*,*,i],Px,Py))
endfor
...
irootJ = fltarr(2,2,M)
for i=0L,4*M-1 do irootJ[i] = 1./sqrt(total(Wi[(i mod 2)*Px/2: $
  (i mod 2+1)*Px/2-1,((i mod 4)/2)*Py/2:((i mod 4)/2+1)*Py/2-1,i/4],/double))

delHD = fltarr(2*P + 4*M + Gamma)
delHD[0:P-1] = total(Sa*Di_H0i,3,/double)
delHD[P:2*P-1] = total(Di_H0i,3,/double)
for i=0L,4*M-1 do delHD[2*P+i] = total(Di_H0i[(i mod 2)*Px/2: $
  (i mod 2+1)*Px/2-1,((i mod 4)/2)*Py/2:((i mod 4)/2+1)*Py/2-1,i/4],/double)
...
for i=0L,Gamma-1 do begin
  ...
  delHD[2*P+4*M+i] = total(Gp[here mod P]*Di_H0i[here],/double)
  pix_ind[here] = i
...
for i=0L,2*M-1 do U[*,*,i] = $
      (irootA[*,*,i/M]*U[*,*,i] + irootA[*,*,i/M+2]*U[*,*,(i+M)mod(M*2)]) $
             *congrid(irootJ[*,*,i mod M],Px,Py)
for i=0L,M-1 do tau[*,*,i] = congrid(irootJ[*,*,i],Px,Py)*tau[*,*,i]*Gp
for i=0L,Gamma-1 do begin
  here = pix_sort[starts[i]:starts[i+1]-1]
...
if (do_ipsimap) then begin
  ...
  for i=0L,Gamma-1 do begin
    ...
    for j=0L,n_elements(here)-1 do begin
      ...
      jj = lindgen(M)*4 + pix_x/(Px/2) + pix_y/(Py/2)*2
      UtT[jj] = UtT[jj] + U[pix_x,pix_y,0:M-1] * T[here[j]] + $
                          U[pix_x,pix_y,M:*] * T[here[j]+PxM]
    endfor
    q_here = (here mod Px)/(Px/2) + (here/Px/(Py/2))*2
...
status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
  irootC*delHD[2*P+4*M:*],Z0,2*P,2*PxM)
...
zeta0 = fltarr(4*M)
status = call_external(c_dir+'spax_tau.so','spax_tau',tau,pix_ind,$
  irootC*delHD[2*P+4*M:*],zeta0,Px,PxM)
zeta0 = -zeta0 + irootJ*delHD[2*P:2*P+4*M-1]
...
if (do_psimap) then begin
  ...
  status = call_external(c_dir+'spax_tau.so','spax_tau',tau,pix_ind,$
                          ualpha,zetapsi0,Px,PxM)
  Zpsin = Zpsi0
  zetapsin = zetapsi0
endif

i = 0L
q_ind = lindgen(Px,2*Py) mod Px/(Px/2) + (lindgen(Px,2*Py)/Px/(Py/2) mod 2)*2
...
while ( ((abs(sZ[i]) gt sZ_limit) and (abs(sZ2[i]) gt sZ2_limit) and $
  ...
  status = call_external(c_dir+'sptx.so','sptx',T,pix_ind2,$
                          Zn,TtZn,2*P,2*PxM)
  status = call_external(c_dir+'sptx_tau.so','sptx_tau',tau,pix_ind,$
                          zetan,tautzetan,Px,PxM)
  status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
                          TtZn+tautzetan,TTtZn,2*P,2*PxM)
  status = call_external(c_dir+'spax_tau.so','spax_tau',tau,pix_ind,$
                          tautzetan+TtZn,tautautzetan,Px,PxM)
  status = call_external(c_dir+'spax_u.so','spax_u',U,q_ind,zetan,Uzetan,2*P,2*PxM)
  status = call_external(c_dir+'sptx_u.so','sptx_u',U,q_ind,Zn,UtZn,2*P,2*PxM)
...
;  dZ1 = dZ1 - total(dZ1,/double)/(2*P)
  ...
  if (do_umap) then begin
    ...
    status = call_external(c_dir+'sptx.so','sptx',T,pix_ind2,$
                          zun,Ttzun,2*P,2*PxM)
    status = call_external(c_dir+'sptx_tau.so','sptx_tau',tau,pix_ind,$
                          zetaun,tautzetaun,Px,PxM)
    status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
                          Ttzun+tautzetaun,TTtzun,2*P,2*PxM)
    status = call_external(c_dir+'spax_tau.so','spax_tau',tau,pix_ind,$
                          tautzetaun+TtZun,tautautzetaun,Px,PxM)
  ...
  if (do_psimap) then begin
    ...
    status = call_external(c_dir+'sptx_tau.so','sptx_tau',tau,pix_ind,$
                          zetapsin,tautzetapsin,Px,PxM)
    status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
                          TtZpsin+tautzetapsin,TTtZpsin,2*P,2*PxM)
    status = call_external(c_dir+'spax_tau.so','spax_tau',tau,pix_ind,$
                          tautzetapsin+TtZpsin,tautautzetapsin,Px,PxM)
    status = call_external(c_dir+'spax_u.so','spax_u',U,q_ind,$

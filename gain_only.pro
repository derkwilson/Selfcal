...
;====================================================================
pro gain_only, Di, astrom, astrom0, map_no, Wi, n_terms, Gp, Fp, Fq, $
...
if (n_elements(umap) eq 1) then $
  if ((umap ge 0) and (umap lt P)) then do_umap=1
do_psimap = 0
...
for i=0L,M-1 do begin
  Di_H0i[*,*,i] = Wi[*,*,i] * (Di[*,*,i]-Gp*Sa[*,*,i]) * cr_flag[*,*,i]
endfor
...
;-----------------------------------------
irootA = 1./sqrt(total(Sa*Wi*Sa,3),/double)
...
;-----------------------------------------
T = temporary(Sa)*temporary(Wi) ; save memory

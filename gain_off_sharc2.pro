...
; procedures.
; 2003-03-25: Modified version of gain_off_4ff.pro to convert the 
; variable detector offsets to variable sky offsets, and to change the 
; number and pattern of variable offsets per frame. The escargot driver
; program must handle the reform steps required for the data.
; This is a slightly tricky way of enabling a data model in which the 
; detector gains and offsets are changing after some number of frames.
...
;====================================================================
pro gain_off_sharc2, Di, astrom, astrom0, map_no, Wi, n_terms, Gp, Fp, Fq, $
...
; Fq         = array (Nsub,M) of variable offsets
;OUTPUT:
...
;-----------------------------------------
sky_size[1:3] = [Px,Py/Nsub,M*Nsub]  ; format to match astrom
pix_no = pixel_numbers(astrom,astrom0,sky_size,map_no,dark_flag,$
           xxs,yys,xo,yo,xpix,ypix,scale=scale,distort_ch=channel)
pix_no = reform(pix_no,Px,Py/Nsub,M,Nsub)
pix_no = transpose(pix_no,[0,1,3,2])
pix_no = reform(pix_no,Px,Py,M)
xpix = reform(xpix,Px,Py/Nsub,M,Nsub)
xpix = transpose(xpix,[0,1,3,2])
xpix = reform(xpix,Px,Py,M)
ypix = reform(ypix,Px,Py/Nsub,M,Nsub)
ypix = transpose(ypix,[0,1,3,2])
ypix = reform(ypix,Px,Py,M)
...
skymap = make_maps(Di,pix_no,pix_sort,xxs,yys,Gp,Fp,wG,varsky=Fq,wtmap)
;h = where(wtmap lt 0.1*max(wtmap),compl=compl)
; for Gal Cent. data
for i=0,n_tags(skymap)-1 do begin
  h = where(wtmap.(i) lt 0.001*max(wtmap.(i)),compl=compl)
  skymap.(i)[h] = median(skymap.(i)[compl])
endfor
wG = 0
...
Di_H0i = Di
findex = lindgen(Px,Py) / Px / (Py/Nsub) 

;if n_elements(Fq) le 1 then Fq = fltarr((size(findex))[1],(size(findex))[2],M)

for i=0L,M-1 do begin
  Di_H0i[*,*,i] = Wi[*,*,i] * cr_flag[*,*,i] * $
       (Di[*,*,i]-Gp*(Sa[*,*,i]+reform(Fq[findex,i],Px,Py))-Fp)
endfor
...
Gp_A = fltarr(Px,Py)
Hp_A = fltarr(Px,Py)
for i=0L,Px-1 do for j=0L,Py-1 do begin
  Gp_A[i,j] = total((Sa[i,j,*]+Fq[findex[i,j],*])^2*Wi[i,j,*],/double)
  Hp_A[i,j] = total((Sa[i,j,*]+Fq[findex[i,j],*])*Wi[i,j,*],/double)
endfor
...
irootJ = fltarr(Nsub,M)
for i=0L,Nsub*M-1 do irootJ[i] = $
  1./sqrt(total(Gp[lindgen(P/Nsub)+(i mod Nsub)*(P/Nsub)]^2*$
  Wi[lindgen(P/Nsub)+i*(P/Nsub)],/double))

delHD = fltarr(2*P + Nsub*M + Gamma)
i0 = lindgen(M)*P
for i=0L,P-1 do delHD[i] = $
  total((Sa[i0+i]+Fq[i/(P/Nsub),*])*Di_H0i[i0+i],/double)
delHD[P:2*P-1] = total(Di_H0i,3,/double)
for i=0L,Nsub*M-1 do delHD[2*P+i] = $
  total(Gp[lindgen(P/Nsub)+(i mod Nsub)*(P/Nsub)]*$
  Di_H0i[lindgen(P/Nsub)+i*(P/Nsub)],/double)
...
for i=0L,Gamma-1 do begin
  ...
  delHD[2*P+Nsub*M+i] = total(Gp[here mod P]*Di_H0i[here],/double)
  pix_ind[here] = i
...
for i=0L,M-1 do T[*,*,i] = T[*,*,i]+Fq[findex,i]*Wi[*,*,i]
tau = temporary(Wi)
U = T
...
for i=0L,2*M-1 do U[*,*,i] = $
      (irootA[*,*,i/M]*U[*,*,i] + irootA[*,*,i/M+2]*U[*,*,(i+M)mod(M*2)]) $
             *reform(irootJ[findex,i mod M],Px,Py)*Gp
for i=0L,M-1 do tau[*,*,i] = reform(irootJ[findex,i],Px,Py)*tau[*,*,i]*Gp^2
findex = 0
for i=0L,Gamma-1 do begin
  here = pix_sort[starts[i]:starts[i+1]-1]
...
if (do_ipsimap) then begin
  ...
  for i=0L,Gamma-1 do begin
    ...
    for j=0L,n_elements(here)-1 do begin
      ...
      UtT[jj] = UtT[jj] + U[pix_x,pix_y,0:M-1] * T[here[j]] + $
                          U[pix_x,pix_y,M:*] * T[here[j]+PxM]
    endfor
    q_here = (here mod 4) + (here/P)*4
    comp1 = total([T[here]^2,T[here+PxM]^2],/double)
...
status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
  irootC*delHD[2*P+Nsub*M:*],Z0,2*P,2*PxM)
...
zeta0 = fltarr(Nsub*M)
status = call_external(c_dir+'spax_tau_nsub.so','spax_tau_nsub',tau,pix_ind,$
  irootC*delHD[2*P+Nsub*M:*],zeta0,P,PxM,Nsub)
zeta0 = -zeta0 + irootJ*delHD[2*P:2*P+Nsub*M-1]
...
i = 0L
q_ind = (lindgen(Px,2*Py) mod P) / (P/Nsub)
...
while ( ((abs(sZ[i]) gt sZ_limit) and (abs(sZ2[i]) gt sZ2_limit) and $
  ...
  tautautzetan = fltarr(Nsub*M)
  Uzetan = fltarr(2*P)
  UtZn = fltarr(Nsub*M)
  status = call_external(c_dir+'sptx.so','sptx',T,pix_ind2,$
                          Zn,TtZn,2*P,2*PxM)
  status = call_external(c_dir+'sptx_tau_nsub.so','sptx_tau_nsub',tau,pix_ind,$
                          zetan,tautzetan,P,PxM,Nsub)
  status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
                          TtZn+tautzetan,TTtZn,2*P,2*PxM)
  status = call_external(c_dir+'spax_tau_nsub.so','spax_tau_nsub',tau,pix_ind,$
                          tautzetan+TtZn,tautautzetan,P,PxM,Nsub)
  status = call_external(c_dir+'spax_u_nsub.so','spax_u_nsub',U,q_ind,$
                          zetan,Uzetan,2*P,2*PxM,Nsub)
  status = call_external(c_dir+'sptx_u_nsub.so','sptx_u_nsub',U,q_ind,$
                          Zn,UtZn,2*P,2*PxM,Nsub)
...
;  dZ1 = dZ1 - total(dZ1,/double)/(2*P)
  ...
  if (do_psimap) then begin
    ...
    status = call_external(c_dir+'spax.so','spax',T,pix_ind2,$
                          TtZpsin+tautzetapsin,TTtZpsin,2*P,2*PxM)
    status = call_external(c_dir+'spax_tau_4ff.so','spax_tau_4ff',tau,pix_ind,$
                          tautzetapsin+TtZpsin,tautautzetapsin,P,PxM)
    status = call_external(c_dir+'spax_u.so','spax_u',U,q_ind,$
                          zetapsin,Uzetapsin,2*P,2*PxM)
    status = call_external(c_dir+'sptx_u.so','sptx_u',U,q_ind,$
                          Zpsin,UtZpsin,2*P,2*PxM)
...
Fq = Fq+xxp
skymap = make_maps(Di,pix_no,pix_sort,xxs,yys,Gp,Fp,Wi*cr_flag,varsky=Fq,wtmap)
...
make_sigs,Di,pix_no,pix_sort,map_no,xxs,yys,Gp,Fp,skymap,Wi,cr_flag,$
  sigmadet,sigmasky,chi2,varsky=Fq,n_sig=n_sig
skymap = make_maps(Di,pix_no,pix_sort,xxs,yys,Gp,Fp,Wi*cr_flag,varsky=Fq,wtmap)
